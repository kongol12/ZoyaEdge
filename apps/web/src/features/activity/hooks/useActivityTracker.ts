  ...
  const logActivity = async (data: {
    ...
    try {
      await addDoc(collection(db, 'system_logs'), {
        ...
        ip: 'client-side', // [DOCUMENTATION] Limitation: L'adresse IP réelle ne peut pas être collectée de manière fiable et sécurisée directement depuis le navigateur client. Idéalement, les logs sensibles devraient passer par une API backend pour capturer req.ip.
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      if (clickable) {
        ...
        let rawLabel = el.innerText?.trim() || 
        ...
        if (rawLabel === 'composante-ui' || rawLabel.length < 1) {
          const svg = el.querySelector('svg');
          if (svg) {
            const lucideClass = Array.from(svg.classList).find(c => c.startsWith('lucide-'));
            if (lucideClass) rawLabel = `Icône ${lucideClass.replace('lucide-', '')}`;
          }
        }
        ...
        logActivity({
          type: 'action',
          severity: 'info',
          message: `Interaction: [${sanitizedLabel}]`,
          path: location.pathname,
          metadata: { 
            element: clickable.tagName.toLowerCase(),
            id: el.id ? el.id.substring(0, 50).replace(/[^\w-]/g, '') : '',
