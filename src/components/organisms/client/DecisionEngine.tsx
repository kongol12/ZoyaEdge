      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-lg">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h2 className="text-base sm:text-lg font-poppins font-black text-gray-900 dark:text-white truncate">ZoyaEdge AI Analysis</h2>
            <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 truncate">
              {data ? `${t.dashboard.lastUpdated}: ${new Date((data as any).updatedAt?.toDate?.() || Date.now()).toLocaleString(language === 'fr' ? 'fr-FR' : 'en-US')}` : t.dashboard.noAnalysis}
            </p>
          </div>
        </div>
        <button
        ...
        >
          ...
          <span>{data ? t.dashboard.refresh : t.dashboard.askCoach}</span>
        </button>
      </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                ...
                {data.actions.length === 0 && (
                  <div className="text-sm text-gray-500 dark:text-gray-400 italic">{t.dashboard.noActions}</div>
                )}
              </div>
            </div>
          </div>
